# 地灵系统 (Earth Spirit) 操作文档

## 1. 系统概述 (Overview)
**地灵系统** 是一个基于 Spigot/Paper API 开发的 Minecraft 伴随/守护实体系统。它通过独特的“双实体架构”（ArmorStand + Wolf）实现了流畅的跟随逻辑和交互体验，并集成了饥饿、心情、等级、背包以及 Towny 城镇插件联动等功能。

系统的核心设计理念是让玩家拥有一个既有“灵魂”（自主行为、情绪反馈）又有“功能”（背包、守护、资源加成）的伙伴。

---

## 2. 核心架构 (Core Architecture)

### 2.1 模块结构

| 模块 | 类文件 | 职责 |
| :--- | :--- | :--- |
| **实体核心** | `SpiritEntity.java` | 定义地灵的所有行为逻辑（移动、状态、渲染、持久化、双实体同步）。 |
| **生命周期管理** | `SpiritManager.java` | 管理地灵数据的加载、保存 (`spirits.yml`)、索引及缓存系统。 |
| **交互监听** | `SpiritListener.java` | 处理玩家输入（右键召唤、GUI 打开、物品交互、进城提示、伤害事件取消）。 |
| **GUI 系统** | `SpiritGUI.java` | 提供可视化操作界面（抚摸、投喂、背包、设置、嘴馋清单、状态查看、居所管理、信任名单、伴侣设置）。 |
| **背包系统** | `SpiritInventory.java` | 处理地灵背包的存储，支持 Base64 序列化与反序列化。 |
| **嘴馋系统** | `CravingManager.java` | 管理每日随机任务请求及奖励结算。 |
| **皮肤管理** | `SpiritSkinManager.java` | 管理自定义头颅纹理、表情切换及 **CustomModelData** 支持。 |
| **任务调度** | `SpiritTask.java` | 全局 Runnable，负责每 tick 驱动所有地灵的更新 (`tick()`)。 |
| **特效渲染** | `SpiritParticleTask.java` | 负责处理环境粒子特效（如富集区金粉、灵域加成蓝火）。 |
| **Towny 集成** | `TownyIntegration.java` | 负责与 Towny 插件的数据交互（自动创建居所、加成判断、权限同步）。 |
| **跨插件支持** | `BiomeGiftsHelper.java` | 通过反射软连接 `BiomeGifts` 插件，实现群系判断和物品获取。 |
| **插件入口** | `EarthSpiritPlugin.java` | 负责插件初始化、指令注册 (`/getbell`, `/getwand`) 及配方注册。 |

### 2.2 依赖关系
*   **Soft Depend**: `Towny` (居所功能), `BiomeGifts` (群系判定与物品)
*   **API**: Spigot/Paper 1.20+

---

## 3. 详细模块解析 (Detailed Module Analysis)

### 3.1 实体核心逻辑 (`SpiritEntity.java`)
这是整个系统最复杂的部分，采用了**双实体驱动 (Dual-Entity Driver)** 方案：
*   **Driver (驱动层)**: 一只隐形、静音、无敌的幼年狼 (`Wolf`)。利用原版 AI 处理寻路和跟随。
*   **Renderer (渲染层)**: 一个小型的盔甲架 (`ArmorStand`)。负责显示头颅、手持物和发光。

#### 关键特性：
1.  **平滑移动**: 使用线性插值 (Lerp) 算法平滑同步 ArmorStand 与 Wolf 的位置，消除卡顿感。
2.  **持久化与恢复**: 
    *   实体支持 **服务器重启后自动恢复**。
    *   采用 **UUID 软链接** 与 **PDC (PersistentDataContainer) 空间搜索** 双重机制，确保即使实体被卸载或轻微位移也能重新找回。
    *   **幽灵实体修复**: 召唤时自动扫描并清理残留的无主“幽灵”实体，防止服务器卡顿。
3.  **外观自定义**:
    *   地灵头颅支持 `CustomModelData: 10004`，允许通过资源包替换为精美的 3D 模型。
4.  **状态系统**:
    *   **Mood (心情)**: 0-100。UI显示为粉色(High)/绿色(Mid)/红色(Low)进度条。影响皮肤表情和加成。
    *   **Hunger (饱食度)**: 随时间消耗。过低会触发“饿饿...饭饭...”悬浮气泡。
    *   **Mode (模式)**: 
        *   `COMPANION`: 跟随主人，提供背包功能。
        *   `GUARDIAN`: 驻守原地（需在居所内），生成底盘，提供范围加成。

### 3.2 交互与物品 (`SpiritListener.java`)
#### 物品识别
*   **灵契风铃 (Bell)**: `CustomModelData: 10001` - 右键召唤/收回地灵。
*   **风铃杖 (Blaze Rod)**: `CustomModelData: 10002` - 右键地面指挥地灵移动/守卫。

#### 投喂机制
*   **动态识别**: 使用 `Material.isEdible()` 兼容所有模组食物。
*   **数值逻辑**: 
    *   基础回复量 = 原版食物回复量。
    *   **饥饿时**: 额外回复大量心情与经验。
    *   **不饿时**: 心情低则加心情，心情高则无额外收益。

### 3.3 居所与 Towny 深度集成 (`TownyIntegration.java`)
*   **一键建家**: 玩家通过地灵界面可直接创建 **居所 (Town)**，自动设置 HomeBlock 和 Spawn。
*   **居所管理**:
    *   **信任名单**: 主人可在 GUI 中添加信任玩家，使其拥有居所权限。
    *   **权限控制**: 自动接管居所的 PVP、爆炸、火蔓延、怪物生成开关（守护模式下自动开启安全保护）。
*   **灵域加成**: 当地灵处于 **GUARDIAN** 模式且心情高涨时，为居所提供 Buff：
    *   **减伤**: 居所内受到的伤害减少 10%-20%。
    *   **生长 (CuisineFarming联动)**: 
        *   提供 **Growth Bonus** API 供 `CuisineFarming` 调用。
        *   加成逻辑：地灵心情 >= 90 提供满额加成 (+10%)。
        *   **缓存优化**: 实现了基于 Chunk 的 `chunkGrowthBonusCache`，缓存时间 **1 分钟**，极大减少了高频生长事件中对 Towny API 的查询压力。
    *   **掉落 (BiomeGifts联动)**:
        *   提供 **Drop Bonus** API 供 `BiomeGifts` 调用。
        *   **机制变更**: 地灵不再直接判定特产掉落，而是提供加成数值（心情 >= 90 时 +10%），由 BiomeGifts 统一进行单次判定。

### 3.4 嘴馋与每日任务 (`CravingManager.java`)
*   **每日刷新**: 每日凌晨 4:00 重置。
*   **任务分级**: C -> S 级，奖励递增。
*   **内容**: 随机需求原版物品或 BiomeGifts 特产（如“热带糖蜜”）。
*   **特殊奖励**: 完成高阶任务概率获得 **灵契之种**，用于种植特殊作物。

### 3.5 数据持久化 (`SpiritManager.java`)
*   **文件**: `spirits.yml`, `requests.yml`
*   **机制**: 
    *   使用 YAML 结构化存储。
    *   保存所有地灵属性、背包内容 (Base64)、信任名单、伴侣关系等。
    *   **容错**: 加载时若世界不存在，会自动处理位置防止崩服。

### 3.6 伴侣系统 (Companion System)
*   **灵魂伴侣**: 玩家可以指定一名其他玩家为“灵魂伴侣”。
*   **操作**: 在居所管理 GUI 中点击 **[❤ 灵魂伴侣]** 发起请求。
*   **特权**: 
    *   伴侣自动获得地灵的 **最高信任权限**。
    *   可以使用地灵背包、管理居所设置。
    *   双方共享居所权限。

---

## 4. 指令与权限 (Commands & Permissions)

| 指令 | 权限 | 描述 |
| :--- | :--- | :--- |
| `/getbell` | `earthspirit.admin` | 获取灵契风铃（管理员用）。 |
| `/getwand` | `earthspirit.admin` | 获取风铃杖（管理员用）。 |

---

## 5. 常见问题排查 (Troubleshooting)

*   **Q: 地灵无法移动/卡住？**
    *   A: 尝试使用灵契风铃收回再重新召唤。新版系统增加了自动纠错机制，重启服务器也能自动恢复连接。
*   **Q: 没有灵域加成效果？**
    *   A: 检查：1. 是否在居所 (Town) 内；2. 地灵是否为 GUARDIAN 模式；3. 地灵心情是否 >= 90。
*   **Q: 无法投喂？**
    *   A: 确保物品可食用。如果地灵属于他人，只有在地灵“抑郁”（被遗弃）状态下才允许陌生人投喂（触发过继逻辑）。
*   **Q: 如何更换地灵模型？**
    *   A: 地灵头颅使用了 `CustomModelData: 10004`，可以通过服务器资源包替换该数据的模型文件。

---

**文档维护人**: 开发组
**最后更新**: 2025-12-21
